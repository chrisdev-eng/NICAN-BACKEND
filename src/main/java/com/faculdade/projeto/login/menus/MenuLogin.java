...
public class MenuLogin {
  ...
  private static void cadastrar(Scanner leitor, Perfil perfil) {
        try {
            ...
            // Pega o admin logado (se houver) para associar ao novo usuario
            Admin adminLogado = null;
            Sessao sessaoAtual = Sessao.get();
            if (sessaoAtual.estaLogado() && sessaoAtual.usuarioEhAdmin()) {
                adminLogado = sessaoAtual.getUsuarioLogado().getAdminResponsavel();
            }
            Usuario novo = new Usuario(nome, login, senha, perfil, adminLogado);
