...
public class TelaLogin extends JFrame {
    ...
    public TelaLogin() {
        NicanTheme.configurarJanela(this, "Login - NICAN", 420, 420);
    ...
    private JPanel criarCorpo() {
        ...
        btnEntrar.addActionListener(e -> {
            ...
            String erro = loginController.fazerLogin(email, senha);
            if (erro != null) {
                NicanDialog.erro(this, erro);
            } else {
                Preferences prefs = Preferences.userRoot();
                prefs.put("ultimoLogin", email);
                if (loginController.ehAdmin()) { new TelaSistemaAdmin(); } else { new TelaHomeUsuario(); }
                dispose();
            }
        });
